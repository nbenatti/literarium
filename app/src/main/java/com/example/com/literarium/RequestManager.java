...
public class RequestManager {
    ...
    public static String formatRequest(RequestType type, Object ... args) throws UnsupportedEncodingException {
        ...
        if(type == RequestType.GEO_REPORT) {
            req = "http://95.232.94.118/literarium_api/geo_report.php?userid=" + URLEncoder.encode(urlEncodedArgs[0], java.nio.charset.StandardCharsets.UTF_8.toString());
        } else if(type == RequestType.LOG_POSITION) {
            req = "http://95.232.94.118/literarium_api/insert_geo_data.php?"
            ...
            req = "http://95.232.94.118/literarium_api/auth.php?"
                    +"nomeutente=" + urlEncodedArgs[0]
                    +"&password=" + urlEncodedArgs[1];
        } else if(type == RequestType.SHARE_BOOK) {

            req = "http://95.232.94.118/literarium_api/share_book.php?"
            ...
            req = "http://95.232.94.118/literarium_api/get_new_shares.php?"
                    + "token=" + urlEncodedArgs[0]
                    + "&userid=" + urlEncodedArgs[1]
                    + "&timestamp=" + urlEncodedArgs[2];
        } else if(type == RequestType.DELETE_SHARE) {

            req = "http://95.232.94.118/literarium_api/delete_share.php?"
                    + "token=" + urlEncodedArgs[0]
                    + "&receiverid=" + urlEncodedArgs[1]
                    + "&bookid=" + urlEncodedArgs[2];
        } else if(type == RequestType.REGISTER) {

            req = "http://95.232.94.118/literarium_api/register.php?"
